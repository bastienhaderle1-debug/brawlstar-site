...
(function () {
    ...
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 480 320">
      <defs>
        <linearGradient id="bg" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stop-color="#1b67dc" />
          <stop offset="100%" stop-color="#0b245f" />
        </linearGradient>
      </defs>
      <rect width="480" height="320" rx="28" fill="url(#bg)" />
      <circle cx="116" cy="92" r="54" fill="rgba(255,255,255,0.12)" />
      <circle cx="370" cy="228" r="72" fill="rgba(255,204,0,0.16)" />
      <text x="50%" y="46%" text-anchor="middle" fill="#ffffff" font-family="Arial, sans-serif" font-size="34" font-weight="700">Skin</text>
      <text x="50%" y="60%" text-anchor="middle" fill="#ffd45c" font-family="Arial, sans-serif" font-size="20">Image indisponible</text>
    </svg>
  `)}`;
  ...
  (async () => {
    ...
    function applyUrlState() {
      ...
      const q = safeStr(params.get("q")).trim();
      const brawler = safeStr(params.get("brawler")).trim();
      const selectedRarity = safeStr(params.get("rarity")).trim();
      const modeParam = safeStr(params.get("mode")).trim().toLowerCase();

      if (modeParam === "theme") {
        mode = "theme";
        modeBrawlerBtn.classList.remove("is-active");
        modeThemeBtn.classList.add("is-active");
        buildOptions();
      }

      if (q) search.value = q;
      if (brawler && [...select.options].some((option) => option.value === brawler)) {
        select.value = brawler;
      }
      if (selectedRarity && [...rarity.options].some((option) => option.value === selectedRarity)) {
        rarity.value = selectedRarity;
      }
    }
